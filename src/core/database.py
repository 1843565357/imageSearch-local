...
class DatabaseManager:
    ...
    def get_images_page(self, offset=0, limit=20):
        """分页获取图片记录"""
        with self.get_connection() as conn:
            cursor = conn.cursor()
            cursor.execute(
                "SELECT id, image_path, description, created_at FROM images ORDER BY created_at DESC LIMIT ? OFFSET ?",
                (limit, offset)
            )
            return cursor.fetchall()
    ...
    def get_statistics(self):
        ...
        with self.get_connection() as conn:
            ...
            # 有特征向量的记录数
            cursor.execute("SELECT COUNT(*) FROM images WHERE vector_data IS NOT NULL")
            ...
            # 最早入库时间
            cursor.execute("SELECT MIN(created_at) FROM images")
            ...
            return {
                ...
                "description_rate": with_description / total if total > 0 else 0,
                "vector_rate": with_vector / total if total > 0 else 0
            }

db_manager = DatabaseManager(db_folder=config.DB_FOLDER if config.DB_FOLDER else "database")